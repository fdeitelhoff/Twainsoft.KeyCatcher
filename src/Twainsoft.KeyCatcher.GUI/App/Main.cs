...
namespace Twainsoft.KeyCatcher.GUI.App
{
    public partial class Main : Form
    {
        ...
        private void LoadStatistics()
        {
            ...
            sessionsRecorded.Text = $"Sessions recorded: {sessionCount}";
            overallKeysCatched.Text = $"Overall keys caught: {caughtKeys}";
        ...
        private void KeyboardCatcherOnSessionStatusChanged(object sender, SessionStatusChangedEventArgs sessionStatusChangedEventArgs)
        {
            ...
            else switch (sessionStatusChangedEventArgs.StatusChange)
            {
                case SessionStatus.Saved:
                        ...
                        ShowBalloonTip("Session saved",
                            $"The session '{sessionStatusChangedEventArgs.KeyboardSession.Name}' with {sessionStatusChangedEventArgs.KeyboardSession.KeyPressCount} keystrokes was stopped and saved. The keyboard input will no longer be caught!",
                            ToolTipIcon.Info);
                    break;
        ...
        {
            KeyboardCatcher.StartSession();
        }

        private void stopSessionMenuItem_Click(object sender, EventArgs e)
        {
            KeyboardCatcher.CancelSession(false);
        }

        private void discardSessionMenuItem_Click(object sender, EventArgs e)
        {
            KeyboardCatcher.DiscardSession();
        }
        ...
        private void ShowMe()
        {
            const string title = "KeyCatcher running";
            const string message = "KeyCatcher is already running and there can be only one instance of this application.";
